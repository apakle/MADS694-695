...
The primary outcome of using unsupervised learning methods on the asteroid data set was disappointing, as clustering did not provide valuable insights. The variables in the original data set were mainly size and velocity, which made it challenging to find unique insights about the asteroids. In a more ideal scenario with more time and resources, we would like to find additional data points or data sets that contain more information about the asteroids, but collecting such data would be nearly impossible.
